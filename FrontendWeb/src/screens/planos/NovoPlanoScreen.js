...
export default function NovoPlanoScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await planoService.criar(dataToSend);
      showSuccess(result.message || 'Plano criado com sucesso');
      router.push('/planos');
    } catch (error) {
      showError(error.errors?.join('\n') || error.error || 'Não foi possível criar o plano');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <LayoutBase title="Novo Plano" subtitle="Cadastrar novo plano de assinatura">
