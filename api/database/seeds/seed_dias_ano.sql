-- Seed para gerar dias para todo o ano
-- Este script preenche a tabela `dias` com datas do ano atual

-- Inserir dias para todo o ano (365 dias a partir de hoje)
INSERT IGNORE INTO dias (data, ativo) 
SELECT 
    DATE_ADD(CURDATE(), INTERVAL i DAY) as data,
    1 as ativo
FROM (
    SELECT 0 as i UNION SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION
...
WHERE DATE_ADD(CURDATE(), INTERVAL i DAY) > CURDATE()
ORDER BY data ASC;

-- Verificar quantos dias foram inseridos
SELECT COUNT(*) as dias_inseridos FROM dias WHERE data >= CURDATE();
