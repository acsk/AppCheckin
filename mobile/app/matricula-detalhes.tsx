  ...
  const loadMatriculaDetails = async () => {
    try {
      ...
      if (data.success) {
        ...
        Alert.alert(
          "Erro",
          data.error || "Não foi possível carregar a matrícula",
        );
      }
    } catch (error: any) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.matriculaHeader}>
          <View
            style={[
              ...
              {
                backgroundColor:
                  matricula.plano.modalidade?.cor || colors.primary,
              },
            ]}
          />
          ...
          <View
            style={[
              ...
              {
                backgroundColor:
                  matricula.status === "ativa" ? "#4CAF50" : "#f44336",
              },
            ]}
          >
          ...
          <View style={styles.dateItem}>
            ...
            <View style={styles.dateContent}>
              ...
              <Text style={styles.dateValue}>
                {new Date(matricula.datas.matricula).toLocaleDateString(
                  "pt-BR",
                )}
              </Text>
            </View>
          ...
          <View style={styles.dateItem}>
            ...
            <View style={styles.dateContent}>
              ...
              <Text style={[styles.dateValue, { color: "#FF9800" }]}>
                {new Date(matricula.datas.vencimento).toLocaleDateString(
                  "pt-BR",
                )}
              </Text>
            </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoValue}>
              R${" "}
              {typeof matricula.plano.valor === "number"
                ? matricula.plano.valor.toFixed(2).replace(".", ",")
                : matricula.plano.valor}
            </Text>
          </View>
            ...
            <View style={styles.resumoGrid}>
              <View style={styles.resumoItem}>
                ...
                <Text style={styles.resumoValue}>
                  R${" "}
                  {typeof resumo.total_previsto === "number"
                    ? resumo.total_previsto.toFixed(2).replace(".", ",")
                    : resumo.total_previsto}
                </Text>
              </View>
              ...
              <View style={[styles.resumoItem, { borderColor: "#4CAF50" }]}>
                ...
                <Text style={[styles.resumoValue, { color: "#4CAF50" }]}>
                  R${" "}
                  {typeof resumo.total_pago === "number"
                    ? resumo.total_pago.toFixed(2).replace(".", ",")
                    : resumo.total_pago}
                </Text>
              </View>
              ...
              <View style={[styles.resumoItem, { borderColor: "#FF9800" }]}>
                ...
                <Text style={[styles.resumoValue, { color: "#FF9800" }]}>
                  R${" "}
                  {typeof resumo.total_pendente === "number"
                    ? resumo.total_pendente.toFixed(2).replace(".", ",")
        ...
        {pagamentos.length > 0 && (
          <View style={styles.card}>
            ...
            {pagamentos.map((pagamento, idx) => {
              const diasRestantes = getDaysUntilExpiry(
                pagamento.data_vencimento,
              );
              const vencidoOuProximo = isExpiringSoon(
                pagamento.data_vencimento,
              );
                    ...
                    <View style={styles.pagamentoDetails}>
                      {vencidoOuProximo ? (
                        <>
                          ...
                          <Text
                            style={[
                              styles.pagamentoDetail,
                              { color: "#FF9800" },
                            ]}
                          >
                            Vence em {diasRestantes} dia
                            {diasRestantes !== 1 ? "s" : ""}
                          </Text>
                        </>
                      ) : (
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            Alert.alert("Pagamento", "Funcionalidade de pagamento em breve!")
          }
        >
