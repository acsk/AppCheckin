...
const getApiUrl = (): string => {
  ...
  // 4️⃣ Produção web: usar o protocolo e host atual
  if (isWeb) {
    const prodUrl = `${window.location.protocol}//${window.location.host}`;
    console.log('📡 API URL (fallback prod):', prodUrl);
    return prodUrl;
  }

  // 5️⃣ Mobile: usar config ou fallback
  console.log('📡 API URL (mobile):', CONFIG.api.development);
  return CONFIG.api.development;
};
