...
class Parametro
{
    ...
    /** Marcador em cache: parâmetro inexistente/inativo (não cachear o $default do caller). */
    private static ?object $cacheMissMarker = null;
    ...
    public function get(int $tenantId, string $codigo, $default = null)
    {
        ...
        if (array_key_exists($cacheKey, self::$cache)) {
            ...
            return $cached === self::cacheMissMarker() ? $default : $cached;
        }
        ...
        if (! $row) {
            self::$cache[$cacheKey] = self::cacheMissMarker();
    ...
    private static function cacheMissMarker(): object
    {
        return self::$cacheMissMarker ??= new \stdClass;
    }
    ...
    public function setMultiple(int $tenantId, array $parametros, ?int $usuarioId = null): bool
    {
        ...
        try {
            foreach ($parametros as $codigo => $valor) {
                if (! $this->set($tenantId, $codigo, $valor, $usuarioId)) {
                    $this->db->rollBack();
