...
class MercadoPagoService
{
    ...
    public function __construct(?int $tenantId = null)
    {
        ...
        $fakeApiUrl = env('MP_FAKE_API_URL');
        ...
        $apiUrl = env('API_URL', 'https://api.appcheckin.com.br');
        $this->notificationUrl = $apiUrl.'/api/webhooks/mercadopago';

        $mobileUrl = env('MOBILE_URL', 'https://mobile.appcheckin.com.br');
    ...
    {
        try {
            $credentials = DB::table('tenant_payment_credentials')
                ->where('tenant_id', $this->tenantId)
                ->where('provider', 'mercadopago')
                ->where('is_active', 1)
                ->first();
            $credentials = $credentials ? (array) $credentials : null;
            ...
            $this->productionMode = $credentials['environment'] === 'production';
    ...
    {
        $this->productionMode = env('MP_ENVIRONMENT', 'sandbox') === 'production';

        if ($this->productionMode) {
            $this->accessToken = env('MP_ACCESS_TOKEN_PROD', '');
            $this->publicKey = env('MP_PUBLIC_KEY_PROD', '');
        } else {
            $this->accessToken = env('MP_ACCESS_TOKEN_TEST', '');
            $this->publicKey = env('MP_PUBLIC_KEY_TEST', '');
        }
    }
    ...
    private function logInicializacao(string $fonte): void
    {
        ...
        error_log("[MercadoPagoService] 🌍 Ambiente: " . ($this->productionMode ? 'PRODUÇÃO' : 'SANDBOX'));
    ...
    private function getExcludedPaymentTypes(?int $tenantId = null): array
    {
        ...
        try {
            $pdo = $this->pdo();
            if (! $pdo) {
                error_log('[MercadoPagoService] ⚠️ Sem conexão PDO — permitindo todos os métodos de pagamento');
    ...
    public function criarPreferenciaPagamento(array $data): array
    {
        ...
        $paymentUrl = $this->productionMode 
            ? $response['init_point'] 
    ...
    public function criarPagamentoPix(array $data): array
    {
        ...
        if ($tenantId) {
            $pdo = $this->pdo();
            if ($pdo) {
                $parametroModel = new Parametro($pdo);
    ...
    public function buscarPagamento($paymentId): array
    {
        ...
        try {
            ...
            return [
                ...
                'payer' => $this->extrairPayer($authorized),
                'payment_method_id' => $this->resolverPaymentMethodId($authorized, 'account_money'),
    ...
    private function normalizarRespostaPagamento(array $response, string $paymentId): array
    {
        return [
            ...
            'payer' => $this->extrairPayer($response),
            'payment_method_id' => $this->resolverPaymentMethodId($response),
     ...
     */
    private function extrairPayer(array $response, bool $incluirId = false): array
    {
        $payer = is_array($response['payer'] ?? null) ? $response['payer'] : [];
    ...
    private function resolverPaymentMethodId(array $response, ?string $fallback = null): ?string
    {
        if (isset($response['payment_method_id']) && $response['payment_method_id'] !== '') {
            return (string) $response['payment_method_id'];
        }
    ...
    public function criarPreferenciaAssinatura(array $data, int $duracaoMeses = 1): array
    {
        ...
        if ($tenantId) {
            $pdo = $this->pdo();
            if (! $pdo) {
                throw new Exception('Não foi possível verificar parâmetros de pagamento.');
            }
        ...
        return [
            ...
            'pagamentos' => array_map(function ($p) {
                return [
                    ...
                    'payer' => $this->extrairPayer($p, true),
                    'metadata' => $p['metadata'] ?? [],
                ];
            }, $results)
        ];
