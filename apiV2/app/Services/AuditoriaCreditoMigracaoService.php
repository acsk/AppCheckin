...
class AuditoriaCreditoMigracaoService
{
    ...
    public function __construct(
        private readonly PagamentoPlanoService $pagamentoPlano,
    ) {}
    ...
    public function auditar(int $tenantId): array
    {
        ...
        foreach ($this->select("
            ...
            WHERE pp.tenant_id = ?
              ...
              -- Conversão ok: crédito ativo/utilizado ligado ao pagamento
              AND NOT EXISTS (
        ...
        $fimPeriodoPago = PagamentoPlanoService::sqlFimPeriodoPago('m.tenant_id', 'm.id');
    ...
    public function repararVencimentoMatricula(int $tenantId, int $matriculaId): array
    {
        ...
        try {
            ...
            ", [$novaData, $novaData, $matriculaId, $tenantId]);
            ...
            ", [$matriculaId, $tenantId]) as $pend) {
                ...
                ", [$novaData, (int) $pend['id'], $matriculaId, $tenantId]);
            }
            ...
            try {
                DB::update("
                    ...
                    WHERE matricula_id = ? AND tenant_id = ?
                ", [$novaData, $matriculaId, $tenantId]);
            } catch (\Throwable) {
                // coluna/tabela opcional em alguns ambientes
            }

            $this->pagamentoPlano->atualizarStatusMatricula($tenantId, $matriculaId);

            $depois = $this->selectOne("
    ...
    private function candidatosVencimento(
        ...
        ", [$matriculaId, $pagamentoId]) as $row) {
            $prox = $row['data_vencimento'] ?? null;
            if ($prox) {
