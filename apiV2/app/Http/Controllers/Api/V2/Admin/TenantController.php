...
class TenantController extends Controller
{
    public function __construct(
        private readonly AdminTenantService $service,
    ) {}

    public function obterWhatsappLinks(Request $request): JsonResponse
    {
        ...
        $result = $this->service->obterWhatsappLinks($tenantId);

        return response()->json($result['body'], $result['status'], [], JSON_UNESCAPED_UNICODE);
    }

    public function salvarWhatsappLinks(Request $request): JsonResponse
    {
        $tenantId = $this->requireTenant($request);
        if ($tenantId instanceof JsonResponse) {
            return $tenantId;
        ...
        $result = $this->service->salvarWhatsappLinks($tenantId, $request->all());
